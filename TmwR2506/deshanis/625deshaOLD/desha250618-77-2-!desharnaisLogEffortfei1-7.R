...
desharnais <- read.table("cleanDesharnaisLogEffort77.csv",
                         sep = ",", header = TRUE)
...
data <- read.csv("/media/user/1907USB/2025github/learnSpector/dashar/!desharnais123fillMedLog71TrainSet.csv")
...
deshaTrain <- subset(deshaTrain,Project!=31)
deshaTrain <- subset(deshaTrain,Project!=41)
deshaTrain <- subset(deshaTrain,Project!=22)
deshaTrain <- subset(deshaTrain,Project!=78)
...
deshaTest <-desha%>%filter(Project%in%c(56,31,41,22,78,73,32,13))
