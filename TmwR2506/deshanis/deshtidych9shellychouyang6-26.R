...
tbDesharnais <- read_csv("desharnais77CH-1.csv",col_names = TRUE,na = '-1',
                         cols(
                           'Project' = col_integer(),
                           'TeamExp' = col_integer(),
...
tbDesharnais_train <- subset(tbDesharnais,Project!=73)
tbDesharnais_train <- subset(tbDesharnais_train,Project!=66)
tbDesharnais_train <- subset(tbDesharnais_train,Project!=56)
...
tbDesharnais_test <- tbDesharnais[c(73,66,56,41,32,22,13), ]
...
#######################################################  +ManagerExp
simple_desh <-
  recipe(Effort ~ TeamExp + PointsNonAdjust , data = tbDesharnais_train) %>%
...
# 预测新数据示例
predict(final_rf_fit, tbDesharnais_test %>% slice(1:7))
