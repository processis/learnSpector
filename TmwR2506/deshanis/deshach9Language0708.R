...
tbDesharnais <- read_csv("/media/user/娱乐/learnSpector/TmwR2506/desharnais77CH-1.csv")%>% 
  filter(Language == 1)
...
# 2. 数据分割
set.seed(123)
split <- initial_split(tbDesharnais, prop = 0.75)
tbDesharnais_train <- training(split)
tbDesharnais_test <- testing(split)
