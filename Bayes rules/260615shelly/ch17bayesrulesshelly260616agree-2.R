setwd("/media/user/软件/learnSpector/Bayes rules/260615shelly")
 # Load packages
library(bayesrules)
library(tidyverse)
library(rstanarm)
             ...
             # Plot runner-specific trends for runners 1 & 10
              ...
              set.seed(84735)
               predict_next_race <- posterior_predict(
                   running_model_1,
                   newdata = data.frame(runner = c("1", "Miles", "8"),
                                                                age = c(5, 1,6)))
               # Posterior predictive model plots
                 mcmc_areas(predict_next_race, prob = 0.8) +
                   ggplot2::scale_y_discrete(labels = c("runner 1", "P11", "runner 6"))
