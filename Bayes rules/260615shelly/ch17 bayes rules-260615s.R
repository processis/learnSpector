...
running<-read_csv("iterDefDen.csv")
...
runner_summaries_2 %>%
  filter(runner %in% c("runner:1", "runner:3"))
...
predict_next_race <- posterior_predict(
  running_model_1,
  newdata = data.frame(runner = c("1", "Miles", "3"),
                       age = c(6, 1, 7)))
...
mcmc_areas(predict_next_race, prob = 0.8) +
  ggplot2::scale_y_discrete(labels = c("runner 1", "Miles", "runner 3"))
