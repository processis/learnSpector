...
grid_data <- data.frame(
  lambda_grid = seq(from = _, to = _, length = _))
# Step 2: Evaluate the prior & likelihood at each lambda
grid_data <- grid_data %>%
  _(prior = dgamma(_, _, _),
      likelihood = dpois(_, _) * dpois(_, _))
# Step 3: Approximate the posterior
grid_data <- grid_data %>%
  _(unnormalized = _,
      posterior = _)
...
"
# STEP 2: SIMULATE the posterior
gp_sim <- _(model_code = _, data = list(_),
              chains = _, iter = _, seed = 84735)
